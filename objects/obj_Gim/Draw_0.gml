...
if (yoTimer and yoTimer <= 45)
{
	...
	draw_sprite_ext(sprHandHoldClose, 0, x, y, dirX, 1, 0, c_white, 1); //values not finalized
}
else if (yoTimer > 45 and yoTimer <= 75)
{
	draw_sprite(spr_Projectile_GimYoyo_Normal, 0, x, y);
	draw_sprite_ext(sprHandHoldAway, 0, x, y, dirX, 1, 0, c_white, 1);
}
else if (yoTimer > 75)
{
	if (instance_exists(throwyo))
	{
		var arr = [sprHandFlingDown, sprHandFlingMiddle, sprHandFlingUp];
