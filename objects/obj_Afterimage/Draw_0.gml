...
switch (owner.object_index)
{
	...
	if (owner.waddleDooEyeFlash) draw_sprite_ext(owner.sprWaddleDooFlashingEye,0,x - (1 * dir),y - 11,owner.image_xscale,owner.image_yscale,owner.imageAngle,owner.image_blend,image_alpha);
	...
	if ((owner.hurt) and (owner.invincibleFlash))
	{
		...
		if ((global.shaders) and (((owner.sprite_index != owner.sprStoneAttack1Common) and (owner.sprite_index != owner.sprStoneAttack1Uncommon) and (owner.sprite_index != owner.sprStoneAttack1Rare)) or (sprite_index = (sprStoneAttack1Common) and (image_index = 0)))) pal_swap_set(paletteIndex,3,false);
